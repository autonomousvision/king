...
from driving_agents.carla.expert.nav_planner import PIDController, RoutePlanner, interpolate_trajectory
...
class AutoPilot(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file, route_index=None):
        ...
        self.clip_throttle = 1.0		# Max throttle (0-1)
        ...
        self.angle_search_range = 25    # Number of future waypoints to consider in angle search
    ...
    def _init(self, hd_map):
        ...
        self._waypoint_planner_extrapolation = RoutePlanner(4.0, 50)
    ...
    def run_step(self, input_data, timestamp):
        ...
        return self._get_control(input_data)
    ...
    def _get_control(self, input_data, steer=None, throttle=None,
        ...
        control = carla.VehicleControl()
        control.steer = np.mean(self.steer_buffer) + 1e-3 * np.random.randn()
        ...
        if((self.step % self.save_freq == 0) and (self.save_path is not None)):
            ...
            far_node, _  = command_route[1] if len(command_route) > 1 else command_route[0]
            if self.render_bev==False:
                tick_data = self.tick(input_data)
            else:
                tick_data = self.tick(input_data, self.future_states)

            self.save(far_node, near_command, steer, throttle, brake, target_speed, tick_data)
