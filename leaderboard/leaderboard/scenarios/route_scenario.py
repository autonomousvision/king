...
SECONDS_GIVEN_PER_METERS = 1.6 # for timeout, default 0.8
...
class RouteScenario(BasicScenario):
    ...
    def _update_route(self, world, config, debug_mode):
        ...
        if debug_mode:
            for scenario in self.sampled_scenarios_definitions:
                loc = carla.Location(scenario['trigger_position']['x'],
                                     scenario['trigger_position']['y'],
                                     scenario['trigger_position']['z']) + carla.Location(z=2.0)
                print(str(scenario['name']))
                world.debug.draw_point(loc, size=0.5, color=carla.Color(255, 0, 0, 255), life_time=100000)
    ...
    def _draw_waypoints(self, world, waypoints, vertical_shift, persistency=-1):
        ...
        for w in waypoints:
            ...
            if w[1] == RoadOption.LEFT:  # Yellow
                ...
                color = carla.Color(255, 64, 0, 255)
            elif w[1] == RoadOption.CHANGELANERIGHT:  # Dark Cyan
                color = carla.Color(0, 64, 255, 255)
            elif w[1] == RoadOption.STRAIGHT:  # Gray
                color = carla.Color(128, 128, 128, 255)
            else:  # LANEFOLLOW
        ...
        world.debug.draw_point(waypoints[0][0].location + carla.Location(z=vertical_shift), size=0.2,
                               color=carla.Color(0, 0, 255, 255), life_time=persistency)
        world.debug.draw_point(waypoints[-1][0].location + carla.Location(z=vertical_shift), size=0.2,
                               color=carla.Color(255, 0, 0, 255), life_time=persistency)
    ...
    def _scenario_sampling(self, potential_scenarios_definitions, random_seed=0):
        ...
        for trigger in potential_scenarios_definitions.keys():
            ...
            if int(os.environ['SAMPLE_SCENARIOS_RANDOMLY']):
                scenario_choice = select_scenario_randomly(possible_scenarios) # modified random sampling
            else:
    ...
    def _initialize_actors(self, config):
        ...
        amount = 5 * town_amount[config.town] if config.town in town_amount else 0
    ...
    def _create_test_criteria(self):
        ...
        blocked_criterion = ActorSpeedAboveThresholdTest(self.ego_vehicles[0],
                                                         speed_threshold=0.1,
                                                         below_threshold_max_time=60.0, # default 180.0
                                                         terminate_on_failure=True,
